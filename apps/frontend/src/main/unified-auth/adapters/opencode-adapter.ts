...
export class OpencodeAuthAdapter implements IProviderAuthAdapter {
  readonly providerType: SharedProviderType = 'opencode';
  ...
  async refreshToken(credentials: ProviderCredentials): Promise<ProviderCredentials> {
    return { ...credentials, providerType: ProviderType.OPENCODE };
  }
