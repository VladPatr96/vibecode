...
export function ChangelogList({
  ...
  const getSummaryLabel = () => {
    ...
    switch (sourceMode) {
      case 'tasks':
        return count === 1 ? t('task') : t('tasks_plural');
      case 'git-history':
      case 'branch-diff':
        return count === 1 ? t('commit') : t('commits_plural');
      default:
        return count === 1 ? t('item') : t('items_plural');
    }
  };
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('tasks.selected', { selected: selectedTaskIds.length, total: doneTasks.length })}
              </span>
            ...
            {doneTasks.length === 0 ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('tasks.noCompleted')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {t('tasks.noCompletedDescription')}
                  </p>
                </div>
              </div>
            ) : (
      ...
      {(sourceMode === 'git-history' || sourceMode === 'branch-diff') && (
        <>
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-muted/30">
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium">
                {t('commits.found', { count: previewCommits.length })}
              </span>
              {isLoadingCommits && (
            ...
            {isLoadingCommits ? (
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <p className="mt-4 text-sm text-muted-foreground">{t('commits.loadingCommits')}</p>
                </div>
              </div>
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center py-12">
                  ...
                  <h3 className="mt-4 text-lg font-medium">{t('commits.noCommits')}</h3>
                  <p className="mt-2 text-sm text-muted-foreground max-w-md">
                    {sourceMode === 'git-history'
                      ? t('commits.noCommitsGitHistory')
                      : t('commits.noCommitsBranchDiff')}
                  </p>
                </div>
              </div>
            ) : (
